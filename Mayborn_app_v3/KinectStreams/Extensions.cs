...
namespace KinectStreams
{
    public static class Extensions
    {
        ...
        public static ImageSource FillBlank(this ColorFrame frame)
        {
            int width = frame.FrameDescription.Width;
            int height = frame.FrameDescription.Height;
            PixelFormat format = PixelFormats.Bgr32;
            ...
            //frame.CopyFrameDataToArray(frameData);
            Array.Clear(frameData, 0, width * height);
        ...
        public static void DrawSkeleton(this Canvas canvas, Body body)
        {
            ...
            canvas.DrawTarget(3, 2, 1, 1, Colors.Red);
            canvas.DrawTarget(3, 2, 2, 1, Colors.Red);
            canvas.DrawTarget(3, 2, 3, 1, Colors.Red);
            canvas.DrawTarget(3, 2, 1, 2, Colors.Red);
            canvas.DrawTarget(3, 2, 2, 2, Colors.Red);
            canvas.DrawTarget(3, 2, 3, 2, Colors.Red);

            canvas.DrawCircle(body.Joints[JointType.HandLeft]);
            canvas.DrawCircle(body.Joints[JointType.HandRight]);
        }
        ...
        public static void DrawCircle(this Canvas canvas, Joint joint)
        {
            if (joint.TrackingState == TrackingState.NotTracked) return;
        ...
        public static void DrawTarget(this Canvas canvas, int m, int n, int a, int b, Color color)
        {
            ...
            Canvas.SetLeft(rect, (1920 / (m + 1) * a) - (rect.Width / 2));
            Canvas.SetTop(rect, (1080 / (n + 1) * b) - (rect.Height / 2));
